...
let titlePage = document.querySelector('.ion-plane')
console.log(title);
...
function startGame(){
  ...
  title.innerHTML = `${name[0].toUpperCase() + name.substr(1).toLowerCase()} vs bot`
...
class Game {
  ...
  humanPlay() {
    return e => {
      ...
      if (this.turnCount >= this.limit) { 
        result.innerHTML = '<span>Draw:</span>'
        return
      }
  ...
  makeAiTurn() {
    ...
    if (this.checkWinner(this.board, aiPlayer)) {
      result.innerHTML = '<span>Bot win:</span>'
      return
    }
  }
